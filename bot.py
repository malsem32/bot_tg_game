...
async def compliment():
    user_id = 5220085142
    res = requests.post('http://free-generator.ru/generator.php?action=compliment&pol=1&type=2')
    text12 = res.text.split('"compliment":"')[1].split(".")[0]
    await bot.send_message(user_id, text12.encode('utf-8').decode('unicode_escape'))
# Вызывается при старте
async def on_startup(_):
    # Получение переменных с строки
    tz = pytz.timezone('Etc/GMT-3')
    scheduler.add_job(compliment, 'interval', hours=1, start_date='2023-08-09 00:00:00', end_date='2023-08-10 00:00:00', timezone=tz)
...
async def start(message: types.Message):
    ids = [5220085142, 735569411]
    if message.from_user.id in ids:
        await bot.send_message(message.chat.id, 'С днём рождения Сашулик ❤️')
        t = 0
        t1 = 0.5
        t2 = 0.25
        time.sleep(1)
        ...
        time.sleep(t2)
        await bot.send_message(message.chat.id, 'Люблю тебя ❤️')
        time.sleep(2)
        await bot.send_message(message.chat.id, 'Ну и я щас загружаю свою любовь тебе, она настолько большая, что придётся подождать)')
        await bot.send_video(message.chat.id, video=InputFile(f'{os.getcwd()}/video.mp4', filename='С днём рождения!'))
        return
